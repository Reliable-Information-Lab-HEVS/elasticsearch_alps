...
STAGE1 COMPATIBLE VERSION - Reduced memory usage and simplified index creation
"""
...
def create_simplified_index_config(num_sources: int) -> Dict[str, Any]:
    """
    ...
    return {
        'settings': {
            ...
            'number_of_shards': 18, # min(17, num_sources * 2)
...
def remote_reindex_merge(source_configs: List[Dict[str, Any]], target_config: Dict[str, Any], 
    ...
    STAGE1 COMPATIBLE - Simplified index creation
    """
    logger = logging.getLogger(__name__)
    ...
    logger.info("=" * 60)
    logger.info("STARTING STAGE1 REMOTE REINDEX MERGE")
    logger.info("=" * 60)
    ...
    try:
        ...
        if target_es.indices.exists(index=target_index, request_timeout=30):
            ...
            # STAGE1 FIX: Use simplified index configuration to avoid startup issues
            config = create_simplified_index_config(len(source_configs))
        ...
        if verification_success:
            logger.info("🎉 STAGE1 MERGE COMPLETED SUCCESSFULLY")
        else:
            logger.warning("⚠️ STAGE1 MERGE COMPLETED WITH ISSUES")
        logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Stage1 Compatible Elasticsearch Index Merger")
