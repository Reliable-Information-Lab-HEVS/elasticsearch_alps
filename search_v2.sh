...
BOOL_MUST_OPERATOR="${BOOL_MUST_OPERATOR:-and}"  # "and" or "or"
BOOL_MUST_MAX_WORDS="${BOOL_MUST_MAX_WORDS:-3}"  # Maximum words to use in bool query
BOOL_MUST_MINIMUM_SHOULD_MATCH="${BOOL_MUST_MINIMUM_SHOULD_MATCH:-}"  # For "or" operator (e.g., "2" or "50%")
...
PATH_DATA="${PATH_DATA:-/iopsstor/scratch/cscs/inesaltemir/es-data-515983-swissai-fineweb-2-quality_33-filterrobots-fineweb_gsw}"

CSV_FILE="${CSV_FILE:-/capstor/scratch/cscs/inesaltemir/scripts/search_WORDS/WeaponizedWords/weaponized_terms_deu.csv}"

INDEX_NAME="${INDEX_NAME:-fineweb_gsw}"


CSV_BASENAME=$(basename "$CSV_FILE" .txt)
OUTPUT_DIR="${OUTPUT_DIR:-/capstor/scratch/cscs/inesaltemir/swissai_search/results/${INDEX_NAME}_${CSV_BASENAME}_${SLURM_JOB_ID}/}"

# Build Elasticsearch URL
ES_URL="http://${ES_HOST}:${ES_PORT}"
# JAVA_OPTS="-Xms31g -Xmx31g -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:G1HeapRegionSize=32m"
# JAVA_OPTS="-Xms31g -Xmx31g -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:G1HeapRegionSize=32m -XX:+UnlockExperimentalVMOptions -XX:+UseCGroupMemoryLimitForHeap"

# Calculate heap based on available memory, but be conservative
if [ "${SLURM_MEM_PER_NODE:-0}" -ge 32768 ]; then
    # 32GB+ available, use 20GB heap (conservative, leaves room for OS + caches)
    JAVA_HEAP="-Xms20g -Xmx20g"
else
...
# Function to build configuration JSON - FIXED VERSION
build_config_json() {
...
start_elasticsearch() {
    ...
    # sudo swapoff -a 2>/dev/null || true SUDO NOT AVAILABLE
    ...
    # export ES_TMPDIR="/tmp/elasticsearch-${SLURM_JOB_ID:-$$}"
    # mkdir -p "$ES_TMPDIR"
   ...
   ES_JAVA_OPTS="$JAVA_OPTS" \
    ...
    max_retries=120  # Increased from 45 to 120 (20 minutes)
    retry_count=0
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        if curl --noproxy "127.0.0.1" -s "http://127.0.0.1:9200/_cluster/health" > /dev/null 2>&1; then
            ...
            if [ $((retry_count % 10)) -eq 0 ]; then
                ...
                log_info "This is normal for 400GB indices - please be patient"
            fi
            sleep 10
        fi
    done
...
# Function to check if index exists with better error handling
check_index_exists() {
    local index_name="$1"
    local http_code
...
main() {
    ...
    # Build configuration JSON - FIXED: Store in variable properly
