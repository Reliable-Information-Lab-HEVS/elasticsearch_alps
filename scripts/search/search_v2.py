...
MODIFIED: Added configurable query execution parameters
SEARCH V2 CODE
"""
...
class ElasticsearchQueryBenchmark:
    def __init__(self, es_url: str, index_name: str, config: Dict[str, Any] = None):
        ...
        self.request_timeout = 60  # Increased timeout
        self.max_retries = 3
    ...
    def match_phrase_query(self, text: str, slop: int = 0) -> Tuple[dict, float]:
        ...
        query = {
            "query": {
                "match_phrase": {
                    "text.exact": match_phrase_config  # FIXED: Back to text.exact
                }
            },
            "size": 50,
            "_source": ["url"],
    ...
    def fuzzy_query(self, text: str) -> Tuple[dict, float]:
        ...
        if not self._is_single_word(text):
            ...
            # More conservative fallback for large index
            query = {
                "query": {
                    "multi_match": {
                        "query": query_text,
                        "fields": ["text"],
                        "fuzziness": "1",  # Limited fuzziness for performance
                        "operator": "and",
    ...
    def extract_hit_snippets(self, hits_data: list, max_hits: int = 5) -> str:
        ...
        for i, hit in enumerate(hits_data[:max_hits]):
            ...
            # CHANGED: Include URL in snippet info
