...
#SBATCH --output=/capstor/scratch/cscs/inesaltemir/INDEXING_TOKENIZED/finemath-3plus-merge/output/indexing_%j.out
#SBATCH --error=/capstor/scratch/cscs/inesaltemir/INDEXING_TOKENIZED/finemath-3plus-merge/err/indexing_%j.err
...
INDEX_NAME="${INDEX_NAME:-finemath-3plus-merge}"     # Index name
...
start_elasticsearch() {
    ...
    # Use JOB_PART_NUM if set, otherwise fallback to just SLURM_JOB_ID
    #if [ -n "$JOB_PART_NUM" ]; then
    #    local job_data_dir="/capstor/store/cscs/swissai/a145/es-data-septemberv1-${INDEX_NAME}-part_${JOB_PART_NUM}-${SLURM_JOB_ID}"
    #    local job_logs_dir="/capstor/store/cscs/swissai/a145/es-logs-septemberv1-${INDEX_NAME}-part_${JOB_PART_NUM}-${SLURM_JOB_ID}"
    #else
    #    local job_data_dir="/capstor/store/cscs/swissai/a145/es-data-septemberv1-${INDEX_NAME}-${SLURM_JOB_ID}"
    #    local job_logs_dir="/capstor/store/cscs/swissai/a145/es-logs-septemberv1-${INDEX_NAME}-${SLURM_JOB_ID}"
    #fi
    if [ -n "$JOB_PART_NUM" ]; then
...
run_indexing() {
    ...
    # Base Python command
    base_cmd="python3 /capstor/scratch/cscs/inesaltemir/scripts/indexing/index_detokenized.py \
        --data-dir \"$DATA_DIR\" \
