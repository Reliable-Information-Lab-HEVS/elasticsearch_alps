...
def calculate_optimal_shards(total_data_size_gb: float, min_shard_size_gb: float = 10.0,
                            max_shard_size_gb: float = 50.0, es_expansion_factor: float = 3.0):
    """Calculate optimal number of shards based on data size"""
    logger = logging.getLogger(__name__)
    
    estimated_indexed_size_gb = total_data_size_gb * es_expansion_factor
    
    logger.info("=" * 45)
    logger.info("SIZE-BASED SHARD CALCULATION")
    logger.info(f"Raw parquet size: {total_data_size_gb:.2f} GB")
    logger.info(f"Estimated indexed size: {estimated_indexed_size_gb:.2f} GB")
    logger.info(f"(using {es_expansion_factor}x expansion factor)")
    ...
    if size_per_shard_gb > max_shard_size_gb:
        optimal_shards = int(estimated_indexed_size_gb / max_shard_size_gb) + 1
        size_per_shard_gb = estimated_indexed_size_gb / optimal_shards
    ...
    if size_per_shard_gb < min_shard_size_gb:
        logger.warning(f"INFO: Size per shard ({size_per_shard_gb:.1f}GB) below min ({min_shard_size_gb}GB)")
...
def process_file_list(file_list, chunk_size, index_name, es, batch_size,
    ...
    doc_queue = Queue(maxsize=80) # 500
...
def main():
    parser = argparse.ArgumentParser(
        description="Index FineWeb dataset into Elasticsearch - Multi-process shared queue architecture"
    )
    ...
    try:
        ...
        # MODIFIED CALL - now includes num_workers parameter
        stats = process_file_list(
            ...
            args.thread_count, 
            args.queue_size,
            args.num_workers  # NEW: Multi-process parameter
        )
