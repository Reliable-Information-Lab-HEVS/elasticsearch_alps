...
#SBATCH --output=/capstor/scratch/cscs/inesaltemir/MERGE_logs/merged_swissai-fineweb-2-quality_33-filterrobots-merge_other-high-merge/output/merge_013_%j.out
#SBATCH --error=/capstor/scratch/cscs/inesaltemir/MERGE_logs/merged_swissai-fineweb-2-quality_33-filterrobots-merge_other-high-merge/error/merge_013_%j.err
...
# MERGE OPERATIONS
# /capstor/scratch/cscs/inesaltemir/scripts/indexing/job_status_logs/fw-other-high/list_dir_fw_other_high.txt
# Targets = 17
# 925967-925983
# 925979 FAILED - RELAUNCH
# merged_swissai-fineweb-2-quality_33-filterrobots-merge_other-high-merge_part_013::: 926805
# SOURCE_DATA_DIRS="/iopsstor/scratch/cscs/inesaltemir/es-data-septemberv1-swissai-fineweb-2-quality_33-filterrobots-merge_other-high_part_029-921756 /iopsstor/scratch/cscs/inesaltemir/es-data-septemberv1-swissai-fineweb-2-quality_33-filterrobots-merge_other-high_part_028-921755"
# SOURCE_INDEX_PATTERNS="swissai-fineweb-2-quality_33-filterrobots-merge_other-high_part"
# TARGET_INDEX="merged_swissai-fineweb-2-quality_33-filterrobots-merge_other-high-merge_part_013"
# MERGE_CLUSTER_PORT="9212" QUE ES ESTO??
...
SOURCE_DATA_DIRS="${SOURCE_DATA_DIRS:-/iopsstor/scratch/cscs/inesaltemir/es-data-septemberv1-swissai-fineweb-2-quality_33-filterrobots-merge_other-high_part_029-921756 /iopsstor/scratch/cscs/inesaltemir/es-data-septemberv1-swissai-fineweb-2-quality_33-filterrobots-merge_other-high_part_028-921755}"
TARGET_INDEX="${TARGET_INDEX:-merged_swissai-fineweb-2-quality_33-filterrobots-merge_other-high-merge_part_013}"
SOURCE_INDEX_PATTERNS="${SOURCE_INDEX_PATTERNS:-swissai-fineweb-2-quality_33-filterrobots-merge_other-high_part}"

# MODIFICATION 1: Hardcoded target port to avoid conflicts
TARGET_PORT=9204
...
configure_proxy_bypass() {
    ...
    export no_proxy="${no_proxy},127.0.0.1,localhost,0.0.0.0,::1,127.0.0.1:9201,127.0.0.1:9202,127.0.0.1:9203,127.0.0.1:9204"
...
show_configuration() {
    ...
    echo "Target Port: $TARGET_PORT (hardcoded)"
    echo "Source Ports: 9201, 9202, 9203 (hardcoded)"
...
start_source_clusters() {
    ...
    # MODIFICATION: Validate 2 or 3 source directories
    if [ ${#source_dirs[@]} -lt 2 ] || [ ${#source_dirs[@]} -gt 3 ]; then
        log_error "This script requires 2 or 3 source directories, got ${#source_dirs[@]}"
        log_error "Please ensure SOURCE_DATA_DIRS contains 2 or 3 space-separated paths"
        return 1
    fi
    ...
    # MODIFICATION: Hardcoded ports for 2 or 3 sources
    local ports=(9201 9202 9203)
    local cluster_pids=()
    
    # Start one cluster per data directory
    for i in "${!source_dirs[@]}"; do
        local data_dir="${source_dirs[$i]}"
        local port="${ports[$i]}"
        ...
        # MODIFICATION: Add reindex.remote.whitelist to source clusters with all possible ports
        ...
        /usr/share/elasticsearch/bin/elasticsearch \
            ...
            -E logger.root=INFO \
            -E reindex.remote.whitelist="127.0.0.1:9201,127.0.0.1:9202,127.0.0.1:9203,127.0.0.1:9204,localhost:*" &
...
start_target_cluster() {
    ...
    # MODIFICATION: Use hardcoded TARGET_PORT
    log_info "Starting target cluster on port $TARGET_PORT (hardcoded)"
    log_info "Target data directory: $target_data_dir"
    
    # Start target cluster optimized for write operations with reindex whitelist for all possible source ports
    ES_JAVA_OPTS="-Xms${heap_size} -Xmx${heap_size}" \
    /usr/share/elasticsearch/bin/elasticsearch \
        ...
        -E discovery.type=single-node \
        -E network.host=127.0.0.1 \
        -E http.port=$TARGET_PORT \
        -E transport.port=$((TARGET_PORT + 100)) \
        -E node.store.allow_mmap=false \
        -E xpack.security.enabled=false \
        -E bootstrap.memory_lock=false \
        -E logger.root=INFO \
        ...
        -E reindex.remote.whitelist="127.0.0.1:9201,127.0.0.1:9202,127.0.0.1:9203,localhost:*" &
    
    export TARGET_ES_PID=$!
    
    # Test connection with proxy bypass
    if ! test_elasticsearch_connection "127.0.0.1" "$TARGET_PORT"; then
        log_error "Target cluster failed to start properly"
        return 1
    fi
    
    log_success "Target cluster ready on port $TARGET_PORT"
    return 0
}
...
discover_and_merge_indexes() {
    ...
    if NO_PROXY="$no_proxy" HTTP_PROXY="" HTTPS_PROXY="" \
       python3 "/capstor/scratch/cscs/inesaltemir/scripts/merge_indexes/merge.py" \
        ...
        local final_count=$(safe_curl -s "http://127.0.0.1:$TARGET_PORT/${TARGET_INDEX}/_count" | grep -o '"count":[0-9]*' | cut -d: -f2 || echo "0")
        local index_exists=$(safe_curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:$TARGET_PORT/${TARGET_INDEX}")
