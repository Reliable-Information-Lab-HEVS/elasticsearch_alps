...
def bulk_index_documents(es: Elasticsearch, doc_generator: Generator, batch_size: int, 
    ...
    try:
        for success, info in helpers.parallel_bulk(
            ...
            request_timeout=300, # was 60 before
        ):
            if success:
                stats["indexed"] += 1
...
def main():
    ...
    try:
        ...
        if indexing_time > 0:
            logger.info(f"Average indexing rate: {stats['indexed'] / indexing_time:.1f} docs/sec") # pq no tengo lo mismo
        ...
        try:
            ...
            logger.info(f"Final index document count: {doc_count:,}") # que es esto
